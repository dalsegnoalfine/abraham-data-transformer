...
<!-- 
     ...
     TODO: Escape quotes from related titles
-->
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="CATFILE/RECORD">
			...
			<xsl:for-each select="HSECTION/LIB/HOLDING">
				...
				<!-- holding_annotations -->
				<xsl:value-of
					select="concat($delimStr,PKNOTE/DATA,$delimStr,$delimL1)" />
		...
		<xsl:for-each select="RELATION[not(@ty='bncl')]">
			...
			<xsl:choose>
				...
				<xsl:when test="@ty = 'oee'">
					<xsl:value-of
						select="concat('Other edition entry',$delimL3,@cloi,$delimL3,DATA)" />
				</xsl:when>
